...
import postsPageInitialPaths1850dResource from '../../resources/posts-page-initial-paths-1850d'
import postsPageInitialProps243ffResource from '../../resources/posts-page-initial-props-243ff'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Lead Program Planner</title>
          <meta property="og:title" content="Posts1 - Lead Program Planner" />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPaths1850dResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps243ffResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
